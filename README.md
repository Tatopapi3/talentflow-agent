...
**Per-vote relay fidelity, verified**: `talentflow_agent`'s final-text divergence bug (see Known limitations) raised the same question for the voting layer — does each vote's stored result faithfully represent what the model actually returned? Checked empirically with `quick_trace_voting.py`, not just assumed structurally, using the same trace methodology proven on the top-level bug: capture each vote's own `agent.messages`, look for a `toolResult` block, and compare it against that vote's final text. Confirmed both structurally and via live trace — `_build_screener_and_prompt`'s `screener` Agent (shared by `_run_screening` and `_run_screening_async`, so this covers both the sync and async screening paths, not just voting) has `tool_names == []`, and every live vote's trace was exactly 2 messages (user prompt, assistant final text) with no `toolUse`/`toolResult` ever appearing — there's no tool-call relay step to diverge at, by construction. Since no `toolResult` exists to compare against, the applicable check is instead whether `strip_to_verdict()` — the one transformation every vote's raw completion goes through before `aggregate_votes()` trusts it — alters anything beyond truncating a preamble before the first `VERDICT:`. Verified `EXACT_MATCH: True` / `EXACT_MATCH_STRIPPED: True` on every vote checked, including through the real `vote_on_resume(n_votes=2)` production path. No bug found; `test_tools.py` case 12 locks this in going forward with both a deterministic structural check (`screener.tool_names == []`) and a live check (`strip_to_verdict()` fidelity against a real completion).
