...
**Per-vote relay fidelity, verified**: `talentflow_agent`'s final-text divergence bug (see Known limitations) raised the same question for the voting layer — does each vote's stored result faithfully represent what the model actually returned? Checked empirically with `quick_trace_voting.py`, not just assumed structurally: `_run_screening_async`'s `screener` Agent has no `tools=` at all (unlike `talentflow_agent`, which wraps `match_resume_to_jd` as a tool and can paraphrase the tool's result in a separate final turn), so each vote's `AgentResult.message` is exactly one text content block — there's no second "agent relays a tool call" hop to diverge at. Verified live, per vote: the raw content block, `str(response)`, and `strip_to_verdict()`'s output are identical (modulo a trailing newline) on every vote checked, including through the real `vote_on_resume(n_votes=2)` production path. No bug found; the voting layer doesn't share `talentflow_agent`'s risk.
